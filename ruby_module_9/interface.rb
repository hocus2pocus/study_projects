...
class Interface
  ...
  def routes_list
    @routes.each.with_index(1) do |route, index|
      puts %(#{index} - начальная станция #{route.stations.first.name},
      конечная станция #{route.stations.last.name}.)
    end
  end
  ...
  def choose_station
    stations_list
    station = take_station_from_user
    if @stations[station]
      @stations[station]
    else
      error_message
